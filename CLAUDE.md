...
│   ├── data/                # DataViewModel (backup export/import + reminder toggle; its UI is hosted on StatsScreen)
...
- **Stats screen** (`ui/stats/`) uses Canvas-based charts (no chart library dependency). Calorie data is aggregated daily for 7-day/30-day views and monthly for the 1-year view. The time range drives a `flatMapLatest` to re-subscribe to both food and weight repositories. Below the charts, a **"Daten" section** (backed by a co-hosted `DataViewModel`) holds the backup Export/Import buttons **and the daily-reminder toggle** — this is the only place these live, since there is no standalone `DataScreen` in the nav graph. A trailing `Spacer` gives the last card breathing room above the bottom nav.
