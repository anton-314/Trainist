...
│   ├── workout/             # "Training" tab — WorkoutScreen (placeholder until workout features land)
...
- **Bottom navigation — four tabs** (`AppNavigation`, `bottomNavItems`): **Ernährung** (`Screen.Overview`, `overview` — the daily food log/overview), **Training** (`Screen.Workout`, `workout` — placeholder for now), **Statistik** (`Screen.Stats`, `stats`), **Einstellungen** (`Screen.Settings`, `settings`). Icons active = `Icons.Rounded.*`, inactive = `Icons.Outlined.*`: Ernährung = `Home`, Training = `FitnessCenter`, Statistik = `BarChart`, Einstellungen = `Settings`. There is **no** standalone Goals tab — goal editing lives in the Settings hub. Any `Screen` route not in `bottomNavItems` (`AddFood`, `BarcodeScanner`) hides the bar.
