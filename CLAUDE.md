...
- **Clean-eating tags** (`domain/model/FoodTag`): every food carries a `FoodTag` — `HEALTHY` (green), `NEUTRAL` (amber), `UNHEALTHY` (red), or the default `NONE` (grey/untagged). Only `HEALTHY` counts as "clean" (`FoodTag.isClean`); untagged and non-healthy kcal count against the clean share, so the ~80–90% target rewards tagging. The tag is stored per `FoodEntry` **and** per custom `Food`, so it only has to be set once and is then pre-filled whenever the food is reused (from history or custom foods) — non-retroactive (editing the tag on one entry does not rewrite past entries).
  - **Where set**: a shared `TagSelector` chip row (`ui/components/FoodTagUi.kt`, mono-uppercase `TAG` caption) in the AddFood amount bottom sheet, the Overview edit dialog, and the custom-food editor. Tapping the selected chip clears back to `NONE`. `TagDot` shows a small colour dot before the food name in every list. Tag colours live in `ui/theme/Color.kt` (`TagHealthyColor`/`TagNeutralColor`/`TagUnhealthyColor`); `FoodTag.color()`/`displayName()` are UI-layer extensions in `FoodTagUi.kt`.
...
- **DataScreen visuals** ("Ink & Paper"): three flat cards (shared `FlatCard`: `surface` fill, 1dp `outlineVariant` hairline border, 0 elevation, 20dp radius) — Export/Import/Tägliche Erinnerung — with serif `titleMedium` titles and sans `bodySmall` descriptions. Export is a filled accent `Button`, Import an `OutlinedButton` (hairline `outlineVariant` border), both 14dp rounded rect. The reminder `Switch` is recolored to a pill (accent `primary` track + `onPrimary` thumb when on; `surfaceVariant`/surface-2 track + `outline` thumb when off, transparent borders). Export/Import functionality and `meal_reminder_enabled` logic are unchanged.
